...
abstract contract ERC1155Abstract is ERCAbstract {
    ...
    ****************************/
    ...
    function _dealUserSingleNft(address user, uint256 tokenId, uint256 tokenBalance) internal {
        ...
        conditionalSkip(cut1155.balanceOf(user, tokenId) != tokenBalance, "Inconclusive test: Issue with dealing NFTs to dummy user.");
    }
    ...
    modifier dealTwoUsersBatchNft(uint256[3] memory tokenIds1, uint256[3] memory tokenBalances1, 
    uint256[3] memory tokenIds2, uint256[3] memory tokenBalances2)
    {
    ...
    ****************************/
    ...
    function _tryAliceSetApprovalForAll(address _operator, bool _approved) 
    internal returns(bool, bool) {
        return _tryCustomerSetApprovalForAll(alice, _operator, _approved);
    }

    function _tryBobSetApprovalForAll(address _operator, bool _approved) 
    internal returns(bool, bool) {
    ...
    function _trySetApprovalForAll(address _operator, bool _approved) 
    internal returns(bool, bool) {
    ...
    function _tryAliceCallsSafeTransferFrom(address _from, address _to, uint256 _id, uint256 _value, bytes calldata _data) 
    internal returns(bool, bool) {
    ...
    function _tryCustomerCallsSafeTransferFrom(address _customer, address _from, address _to, uint256 _id, uint256 _value, bytes calldata _data) 
    internal returns(bool, bool) {
        vm.startPrank(_customer);
    ...
    internal returns(bool, bool) {
        ...
        bytes memory data = abi.encodeWithSignature("safeTransferFrom(address,address,uint256,uint256,bytes)", _from, _to, _id, _value, _data);
        (bool success, bytes memory returnData) = address(cut1155).call(data);
        bool returnValue = false;
        if (success && returnData.length > 0) {
            returnValue = abi.decode(returnData, (bool));
    ...
    function _tryAliceCallsSafeBatchTransferFrom(address _from, address _to, uint256[3] memory _ids, uint256[3] memory _values, bytes calldata _data) 
    internal returns(bool, bool) {
        return _tryCustomerCallsSafeBatchTransferFrom(alice, _from, _to, _ids, _values, _data);
    }

    function _tryBobCallsSafeBatchTransferFrom(address _from, address _to, uint256[3] memory _ids, uint256[3] memory _values, bytes calldata _data) 
    internal returns(bool, bool) {
        return _tryCustomerCallsSafeBatchTransferFrom(bob, _from, _to, _ids, _values, _data);
    }

    function _tryCustomerCallsSafeBatchTransferFrom(address _customer, address _from, address _to, uint256[3] memory _ids, uint256[3] memory _values, bytes calldata _data) 
    internal returns(bool, bool) {
        vm.startPrank(_customer);
    ...
    function _trySafeBatchTransferFrom(address _from, address _to, uint256[3] memory _ids, uint256[3] memory _values, bytes calldata _data) 
    internal returns(bool, bool) {
        ...
        bytes memory data = abi.encodeWithSelector(cut1155.safeBatchTransferFrom.selector, _from, _to, _dynamicIds, _dynamicValues, _data);
        (bool success, bytes memory returnData) = address(cut1155).call(data);
        bool returnValue = false;
        if (success && returnData.length > 0) {
            returnValue = abi.decode(returnData, (bool));
    ...
    function _trySafeBatchTransferFromDynamic(address _from, address _to, uint256[] memory _ids, uint256[] memory _values, bytes calldata _data) 
    internal returns(bool, bool) {
        bytes memory data = abi.encodeWithSelector(cut1155.safeBatchTransferFrom.selector, _from, _to, _ids, _values, _data);
        (bool success, bytes memory returnData) = address(cut1155).call(data);
        bool returnValue = false;
        if (success && returnData.length > 0) {
            returnValue = abi.decode(returnData, (bool));
    ...
    ****************************/
    ...
    function _propertySafeTransferFromIncreaseReceiverBalance(bool toContract, ArithmeticOperator arithmeticOperator, 
    uint256 tokenId, uint256 tokenSenderBalance, uint256 tokenReceiverBalance, uint256 tokenAmount, bytes calldata data) 
    internal 
    {
        ...
        conditionalSkip(!callSuccess, "Inconclusive test: Calling `safeTransferFrom` from Alice to tokenReceiver by Bob reverts unexpectedly.");
        if (arithmeticOperator == ArithmeticOperator.Equal) {
            assertEq(cut1155.balanceOf(toAddress, tokenId), tokenReceiverBalance + tokenAmount, "Balance of tokenId for tokenReceiver does not increase as expected.");
        }
        else if (arithmeticOperator == ArithmeticOperator.Lesser) {
            assertLt(cut1155.balanceOf(toAddress, tokenId), tokenReceiverBalance + tokenAmount, "Balance of tokenId for tokenReceiver increases >= expected.");
        }
        else if (arithmeticOperator == ArithmeticOperator.Greater) {
            assertGt(cut1155.balanceOf(toAddress, tokenId), tokenReceiverBalance + tokenAmount, "Balance of tokenId for tokenReceiver increases <= expected.");
        }
    }
    ...
    /// or more than expected, according to the operator in `arithmeticOperator`.
    /// NOTE: If `testSetApprovalForAllNotRevert` fails, then the result of this test is inconclusive.
    function _propertySafeTransferFromDecreaseOwnerBalance(bool toContract, ArithmeticOperator arithmeticOperator, uint256 tokenId, uint256 tokenBalance, uint256 tokenAmount, bytes calldata data) 
    internal 
    {
        ...
        conditionalSkip(!callSuccess, "Inconclusive test: Calling `safeTransferFrom` from Alice to tokenReceiver by Bob reverts unexpectedly.");        
        if (arithmeticOperator == ArithmeticOperator.Equal) {
            assertEq(tokenBalance - cut1155.balanceOf(alice, tokenId), tokenAmount, "Balance of tokenId for Alice does not decrease as expected.");
        }
        else if (arithmeticOperator == ArithmeticOperator.Lesser) {
            assertLt(tokenBalance - cut1155.balanceOf(alice, tokenId), tokenAmount, "Balance of tokenId for Alice decreases >= expected.");
        }
        else if (arithmeticOperator == ArithmeticOperator.Greater) {
            assertGt(tokenBalance - cut1155.balanceOf(alice, tokenId), tokenAmount, "Balance of tokenId for Alice decreases <= expected.");
        }
    }
    ...
    /// or more than expected, according to the operator in `arithmeticOperator`.
    function _propertySelfSafeTransferFromSelfToSelfChangeOwnerBalance(ArithmeticOperator arithmeticOperator, uint256 tokenId, uint256 tokenBalance, uint256 tokenAmount, bytes calldata data) 
    internal 
    {
        vm.assume(tokenAmount <= tokenBalance);
        ...
        if (!callSuccess) {
            ...
            // Skip the test if the safeTransferFrom call fails
            conditionalSkip(!callSuccessAgain, "Inconclusive test: Calling `safeTransferFrom` from Alice to herself by herself reverts unexpectedly.");
        }
        // The following code entails the part where `safeTransferFrom` is called successfully as expected
        if (arithmeticOperator == ArithmeticOperator.Equal) {
            assertEq(cut1155.balanceOf(alice, tokenId), tokenBalance, "Balance of tokenId for Alice changes unexpectedly.");
        }
        else if (arithmeticOperator == ArithmeticOperator.Lesser) {
