...
abstract contract ERC4626Abstract is ERCAbstract {
    ...
    ****************************/
    ...
    function assertApproxGeAbs(uint a, uint b, uint256 maxDelta) internal {
        if (!(a >= b)) {
            assertGe(b - a, maxDelta);
        }
    }

    function assertApproxGeAbs(uint a, uint b, uint256 maxDelta, string memory err) internal {
        if (!(a >= b)) {
            assertGe(b - a, maxDelta, err);
        }
    }

    function assertApproxLeAbs(uint a, uint b, uint256 maxDelta) internal {
        if (!(a <= b)) {
            assertLe(a - b, maxDelta);
        }
    }

    function assertApproxLeAbs(uint a, uint b, uint256 maxDelta, string memory err) internal {
        if (!(a <= b)) {
    ...
    ****************************/
    ...
    modifier initializeAssetsTwoUniqueNonZeroAddresses(address user1, address user2, uint256 user1AssetsBalance, uint256 user2AssetsBalance) 
    {
    ...
    modifier initializeSharesTwoUniqueNonZeroAddresses(address user1, address user2, uint256 user1SharesBalance, uint256 user2SharesBalance) 
    {
        vm.assume(user1 != address(0x0));
        vm.assume(user2 != address(0x0));
        vm.assume(user1 != user2);
    ...
    ****************************/
    ...
    function tryApproveApproveeAssets(address tokenApprovee, uint256 amount) 
    ...
    function tryTransferReceiverAssets(address tokenReceiver, uint256 amount) 
    internal returns(bool, bool) {
    ...
    function tryCallerTransferFromSenderToReceiverAssets(address caller, address tokenSender, address tokenReceiver, uint256 amount) 
    internal returns(bool, bool) {
        vm.startPrank(caller);
    ...
    ****************************/
    ...
    function tryApproveApproveeShares(address tokenApprovee, uint256 amount) 
    internal returns(bool, bool) {
    ...
    function tryTransferReceiverShares(address tokenReceiver, uint256 amount)
    internal returns(bool, bool) {
    ...
    function tryCallerTransferFromSenderToReceiverShares(address caller, address tokenSender, address tokenReceiver, uint256 amount)
    internal returns(bool, bool) {
        vm.startPrank(caller);
    ...
    ****************************/
    ...
    function tryCallerCallName(address caller)
    internal returns(bool callSuccess, string memory returnValue) {
        vm.startPrank(caller);
    ...
    /// Note: This function does not do any assumption check
    function tryCallerCallSymbol(address caller)
    internal returns(bool callSuccess, string memory returnValue) {
        vm.startPrank(caller);
    ...
    /// Note: This function does not do any assumption check
    function tryCallerCallVaultDecimals(address caller)
    internal returns(bool callSuccess, uint8 returnValue) {
        vm.startPrank(caller);
    ...
    /// Note: This function does not do any assumption check
    function tryCallerCallAssetDecimals(address caller)
    internal returns(bool callSuccess, uint8 returnValue) {
        vm.startPrank(caller);
    ...
    /// Note: This function does not do any assumption check
    function tryCallerCallAsset(address caller)
    internal returns(bool callSuccess, address returnValue) {
        vm.startPrank(caller);
    ...
    /// Note: This function does not do any assumption check
    function tryCallerCallTotalAssets(address caller)
    internal returns(bool callSuccess, uint256 returnValue) {
        vm.startPrank(caller);
    ...
    function tryCallConvertToSharesAssets(uint256 assets)
    internal returns(bool, uint256) {
    ...
    function tryCallConvertToAssetsShares(uint256 shares)
    internal returns(bool, uint256) {
    ...
    function tryCallMaxDepositReceiver(address receiver)
    internal returns(bool, uint256) {
    ...
    function tryCallPreviewDepositAssets(uint256 assets)
    internal returns(bool, uint256) {
    ...
    function tryCallMaxWithdrawOwner(address owner)
    internal returns(bool, uint256) {
    ...
    function tryCallPreviewWithdrawAssets(uint256 assets)
    internal returns(bool, uint256) {
    ...
    function tryCallMaxMintReceiver(address receiver)
    internal returns(bool, uint256) {
    ...
    function tryCallPreviewMintShares(uint256 shares)
    internal returns(bool, uint256) {
    ...
    function tryCallMaxRedeemOwner(address owner)
    internal returns(bool, uint256) {
    ...
    function tryCallPreviewRedeemShares(uint256 shares)
    internal returns(bool, uint256) {
    ...
    ****************************/
    ...
    function tryDepositAssetsToReceiver(uint256 assets, address receiver)
    internal returns(bool, uint256) {
    ...
    internal returns(bool callDeposit, uint256 mintedShares) {
        vm.startPrank(caller);
    ...
    internal assetsOverflowRestriction(assets) returns(bool callDeposit, uint256 mintedShares) {
    ...
    internal returns(bool callWithdraw, uint256 burnedShares) {
        vm.startPrank(caller);
    ...
    function tryCallerWithdrawAssetsToReceiverFromOwnerWithChecksAndApproval(address caller, uint256 assets, address receiver, address owner)
    ...
    internal assetsOverflowRestriction(assets) returns(bool callWithdraw, uint burnedShares) {
        uint256 ownerShares = cut4626.balanceOf(owner);
        // sharesOverflowRestriction on vault.balance[owner]
        if (cut4626.totalSupply() > 0) { 
            vm.assume(ownerShares < MAX_UINT256 / (cut4626.totalAssets() + 1)); 
    ...
    function tryMintSharesToReceiver(uint256 shares, address receiver)
    internal returns(bool, uint256) {
    ...
    internal returns(bool callMint, uint256 depositedAssets) {
        vm.startPrank(caller);
    ...
    internal sharesOverflowRestriction(shares) returns(bool callMint, uint256 depositedAssets) {
        uint256 callerAssets = asset.balanceOf(caller);
    ...
    internal returns(bool callRedeem, uint256 redeemedAssets) {
        vm.startPrank(caller);
    ...
    function tryCallerRedeemSharesToReceiverFromOwnerWithChecksAndApproval(address caller, uint256 shares, address receiver, address owner)
