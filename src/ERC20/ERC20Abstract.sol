...
abstract contract ERC20Abstract is ERCAbstract {
    ...
    ****************************/
    ...
    function _updateContractOwner() internal returns (bool) {
        string[8] memory signatures = ["owner()", "tokenManager()", "newOwner()", "ico()", "admin()", "admin1()", "manager()", "controller()"];
        bool updated = false;
        uint8 i = 0;
    ...
    ***********************************************************************************/
    ...
    function _getVariableValue(string memory variableName) internal returns (bool, bytes memory) {
        (bool success, bytes memory returnData) = address(cut).call(abi.encodeWithSignature(string(abi.encodePacked(variableName, "()"))));
    ...
    ****************************/
    ...
    function _tryCustomerTokenTransferProxy(address customer, address _from, address _to, uint256 _value, uint256 _feeMesh, uint8 _v, bytes32 _r, bytes32 _s)
    internal returns (CallResult memory) {
    ...
    function _tryAliceTransfer(address tokenReceiver, uint256 amount)
    internal returns (CallResult memory) {
        return _tryCustomerTransfer(alice, tokenReceiver, amount);
    }

    /// @notice Abstracts away an approval made by Alice.
    function _tryAliceApprove(address tokenApprovee, uint256 amount)
    internal returns (CallResult memory) {
    ...
    function _tryAliceTokenTransferProxy(address _from, address _to, uint256 _value, uint256 _feeMesh, uint8 _v, bytes32 _r, bytes32 _s)
    internal returns (CallResult memory) {
    ...
    function _tryBobTransfer(address tokenReceiver, uint256 amount)
    internal returns (CallResult memory) {
        return _tryCustomerTransfer(bob, tokenReceiver, amount);
    }

    /// @notice Abstracts away an approval made by Bob.
    function _tryBobApprove(address tokenApprovee, uint256 amount)
    internal returns (CallResult memory) {
    ...
    function _tryBobTokenTransferProxy(address _from, address _to, uint256 _value, uint256 _feeMesh, uint8 _v, bytes32 _r, bytes32 _s)
    internal returns (CallResult memory) {
    ...
    function _tryCarolTokenTransferProxy(address _from, address _to, uint256 _value, uint256 _feeMesh, uint8 _v, bytes32 _r, bytes32 _s)
    internal returns (CallResult memory) {
    ...
    function _tryTransfer(address tokenReceiver, uint256 amount)
    internal returns (CallResult memory) {
    ...
    /// @dev The function performs low-level external call to the test-subject contract and 
    /// manually evaluate the success state and return value (if any) of the call.
    function tryApprove(address tokenApprovee, uint256 amount) 
    internal returns (CallResult memory) {
    ...
    ****************************/
    ...
    function _tryBurnTokensAmount(address burner, uint256 amountToBurn) internal returns (CallResult memory){
        ...
        for (uint8 i = 0; i < burnFunctionNames.length; i++) {
            ...
            CallResult memory callResult = _tryBurnTokensWithFunctionAmount(_concatenate(name, sig), burner, amountToBurn);
    ...
    function _tryBurnTokensAddressAmount(address burningAddress, address burnedAddress, uint256 amountToBurn) internal returns (CallResult memory){
        ...
        for (uint8 i = 0; i < burnFunctionNames.length; i++) {
            ...
            //string memory sig = burnFunctionSignatures[i];
            string memory sig = burnFunctionSignature;
            CallResult memory callResult = _tryBurnTokensWithFunctionAddressAmount(_concatenate(name, sig), burningAddress, burnedAddress, amountToBurn);
            if (callResult.success) {
                return callResult;
            }
        }
    ...
    function _tryBurnTokensWithFunctionAddressAmount(string memory signature, address burningadddress, address _burnedaddress, uint256 amount)
        internal returns (CallResult memory) {
        bytes4 selector = selectorOf(signature);
    ...
    function _mintTokenToAddress(uint256 amountToMint, address receiver) internal returns (CallResult memory){
        ...
        for (i = 0; i < mintFunctionNames.length; i++) {
            ...
            CallResult memory callResult = _tryMintTokensToAddressWithFunction(_concatenate(name, sig), amountToMint, receiver);
            if (callResult.success) {
                return callResult;
            }
        }
    ...
    /// @dev The function performs low-level external call to the test-subject contract and
    /// manually evaluate the success state and return value (if any) of the call.
    function _tryIncreaseAllowance(address tokenApprovee, uint256 amount)
    internal returns (CallResult memory) {
    ...
    function _tryAliceIncreaseAllowance(address tokenApprovee, uint256 amount)
        internal returns (CallResult memory) {
        return _tryCustomerIncreaseAllowance(alice, tokenApprovee, amount);
    }

    /// @notice Abstracts away an allowance increase made by Bob.
    function _tryBobIncreaseAllowance(address tokenApprovee, uint256 amount)
        internal returns (CallResult memory) {
    ...
    /// @dev The function performs low-level external call to the test-subject contract and
    /// manually evaluate the success state and return value (if any) of the call.
    function _tryDecreaseAllowance(address tokenApprovee, uint256 amount)
    internal returns (CallResult memory) {
    ...
    function _tryAliceDecreaseAllowance(address tokenApprovee, uint256 amount)
        internal returns (CallResult memory) {
        return _tryCustomerDecreaseAllowance(alice, tokenApprovee, amount);
    }

    /// @notice Abstracts away an allowance decrease made by Bob.
    function _tryBobDecreaseAllowance(address tokenApprovee, uint256 amount)
        internal returns (CallResult memory) {
    ...
    ***********************************************************************************/
    ...
    function _tryEnableTokenTransfer(address enabler) 
        internal returns (CallResult memory) {
    ...
    function _tryDisableTokenTransfer(address enabler) 
        internal returns (CallResult memory) {
        vm.startPrank(enabler);
    ...
    ***********************************************************************************/
    ...
    /// @dev The function performs low-level external call to the test-subject contract and 
    /// manually evaluate the success state and return value (if any) of the call.
    function _tryBatchTransfer(address[] memory tokenReceivers, uint256 amount)
    internal returns (CallResult memory) {
    ...
    function _trySell(uint256 amount)
    internal returns (CallResult memory) {
